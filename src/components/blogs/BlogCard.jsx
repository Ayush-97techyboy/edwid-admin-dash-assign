...
const BlogCard = ({ blog, isTrash, onEdit, onDelete, onRestore }) => {
  ...
  return (
  <div className={`theme-bg-secondary rounded-xl shadow-sm border theme-border overflow-hidden hover:shadow-xl transition-all group flex flex-col h-full ${isTrash ? 'h-auto' : ''}`}>
    ...
    <div className={`flex-1 flex flex-col ${isTrash ? 'p-3' : 'p-5'}`}>
        ...
        {!isTrash && <p className="text-sm theme-text-secondary mb-4 line-clamp-2">{blog.description}</p>}
        <div className="mt-auto flex justify-between items-center gap-2">
            {!isTrash && <span className="text-xs theme-text-secondary">{t.views}: {(blog.views || 0).toLocaleString()}</span>}
            <div className={`flex ${isTrash ? 'gap-1' : 'gap-2'}`}>
                {!isTrash ? (
                    <>
                        <button onClick={onEdit} className="p-2 hover:bg-primary-50 rounded-lg theme-text-secondary hover:text-primary-600 transition-colors" title="Edit"><FileText size={18}/></button>
